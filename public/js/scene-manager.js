...
// ============================================
// BULLY FORMATION CONSTANTS
// ============================================
const BULLY_CONFIG = {
  // Spacing between players in the front row (shoulder to shoulder, bound)
  playerSpacing: 0.8,        // Meters between player centers when bound
  
  // Front row positions (6 players: 3 attackers, 3 defenders facing each other)
  // Post is in the middle, Side Posts on either side, Corners on the outside
  frontRowOrder: ['c1', 'sp1', 'post', 'sp2', 'c2'],  // From one side to other
  
  // Distance between attacking and defending front rows
  frontRowGap: 0.6,          // Gap where ball sits and heads interlock
  
  // Bup (back-up post) position behind the post
  bupOffsetX: 1.2,           // How far behind the post
  
  // Engagement offset - crouched players lean forward into defenders
  engagementOffset: 0.3
};
...
const SceneManager = {
  ...
  /**
   * Creates a label sprite for a player
   */
  createPlayerLabel(text, color, highlighted) {
    ...
    // Background
    ctx.fillStyle = highlighted ? '#f1c40f' : `#${color.toString(16).padStart(6, '0')}`;
  ...
  updateBall(scene, ballData, animate) {
    ...
    pos3d.y = 0.11; // Ball radius above ground
